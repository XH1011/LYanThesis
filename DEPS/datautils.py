...
class ZSLDataset(Dataset):
    def __init__(self, dset, n_train, n_test, train=True, gzsl=False, synthetic=False, syn_dataset=None):
        ...
        res101_data = scio.loadmat('D:/Desktop/毕业论文/codes/[Step4]ZSL/data/ours/res101_%s.mat' % dset)
        self.features = res101_data['features'].T
        self.labels = res101_data['labels'].reshape(-1).T
        # self.valueall = len(self.labels)


        self.attribute_dict = scio.loadmat('D:/Desktop/毕业论文/codes/[Step4]ZSL/data/ours/att_splits_%s.mat' % dset)
        self.attributes = self.attribute_dict['att'].T

        # file with all class names for deciding train/test split
        self.class_names_file = 'D:/Desktop/毕业论文/codes/[Step4]ZSL/data/ours/classes_%s.txt' % dset


        # test class names
        with open('D:/Desktop/毕业论文/codes/[Step4]ZSL/data/ours/testclasses_%s.txt' % dset) as fp:
