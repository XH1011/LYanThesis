...
class Trainer:
    def __init__(self, device, dset, x_dim, c_dim, z_dim, n_train, n_test, lr, layer_sizes, **kwargs):
        ...
        self.x_encoder = Encoder(x_dim, layer_sizes['x_enc'], z_dim).to(self.device) #(2048,64)
        self.x_decoder = Decoder(z_dim, layer_sizes['x_dec'], x_dim).to(self.device) #(64,2048)

        self.c_encoder = Encoder(c_dim, layer_sizes['c_enc'], z_dim).to(self.device) #(102,64)
        self.c_decoder = Decoder(z_dim, layer_sizes['c_dec'], c_dim).to(self.device) #(64,102)

        self.support_classifier = Classifier(z_dim, self.n_train).to(self.device) #(64,645)
        ...
        self.final_classifier = Classifier(z_dim, self.n_test).to(self.device) #(64,72)
        ...
        self.vae_save_path = './models/'
        self.disc_save_path = './models/disc_model_%s.pth' % self.dset
    ...
    def fit_VAE(self, x, c, y, ep):
        ...
        x = Variable(x.float()).to(self.device) #(50,128)
        c = Variable(c.float()).to(self.device) #(50,30)
        y = Variable(y.long()).to(self.device)  #(50,)

        # VAE for image embeddings
        mu_x, logvar_x = self.x_encoder(x) #(50,64)
        z_x = self.reparameterize(mu_x, logvar_x) #(50,64)
        x_recon = self.x_decoder(z_x) ##(50,128)

        # VAE for class embeddings
        mu_c, logvar_c = self.c_encoder(c) #(50,64)
        z_c = self.reparameterize(mu_c, logvar_c) #(50,64)
        c_recon = self.c_decoder(z_c) #(50,102)
    ...
    def create_syn_dataset(self, test_labels, attributes, seen_dataset, n_samples=400):
        ...
        for test_cls, idx in test_labels.items():
            ...
            self.c_encoder.eval()
            c = Variable(torch.FloatTensor(attr).unsqueeze(0)).to(self.device) #(1,102)
            mu, log_var = self.c_encoder(c) #(1,64)

            Z = torch.cat([self.reparameterize(mu, log_var) for _ in range(n_samples)]) #(400,64)
    ...
    def compute_accuracy(self, generator):
        ...
        acc = accuracy_score(y_real_list, y_pred_list)
        ...
        if self.gzsl:
            ...
            return accuracy_score(y_real_list, y_pred_list),y_real_list,y_pred_list
